...
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			# Configuration
			sed -i 's|# interface: 0.0.0.0$|interface: 10.10.10.1|' /etc/unbound/unbound.conf
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: 10.10.10.1/24 allow|' /etc/unbound/unbound.conf
...
function installQuestions() {
	echo "---------------------------------------------------------------"
	echo "Chào mừng bạn đến cài đặt OpenVPN"
	echo "Thông tin đã được biên dịch System.info.vn"
	echo "---------------------------------------------------------------"
	...
	if echo "$IP" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		echo ""
        echo "Máy chủ này đứng sau NAT. Vui lòng cung cấp địa chỉ IPv4 public hoặc domain server ?"
	...
	if eval "$PING6"; then
		echo "Server của bạn có kết nối IPv6"
		SUGGESTION="y"
	else
		echo "Server của bạn không có kết nối IPv6"
		SUGGESTION="n"
	fi
	echo ""
	# Ask the user if they want to enable IPv6 regardless its availability.
	until [[ $IPV6_SUPPORT =~ (y|n) ]]; do
		read -rp "Bạn có muốn bật IPv6 không (NAT)? [y/n]: " -e -i $SUGGESTION IPV6_SUPPORT
	done
	echo ""
	echo "---------------------------------------------------------------"
	echo "Vui lòng chọn port OpenVPN "
	echo "---------------------------------------------------------------"
	echo ""
	echo "   1) Mặc định: 1194"
	echo "   2) Custom"
	...
	echo ""
	echo "---------------------------------------------------------------"
	echo "Bạn muốn sử dụng giao thức kết nối nào ?"
	echo "Mặc định là UDP. UDP sẽ nhanh hơn TCP."
	echo "---------------------------------------------------------------"
	echo "   1) UDP"
	echo "   2) TCP"
	...
	echo ""
	echo "Bạn muốn sử dụng DNS nào trên OpenVPN?"
	...
	echo ""
	echo "---------------------------------------------------------------"
	echo "Bạn có muốn tùy chỉnh cài đặt mã hóa không?"
	echo "Mặc dịch là: AES-128-GCM"
	echo "---------------------------------------------------------------"
	echo ""
	...
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		echo ""
		echo "Vui lòng chon kiểu mã hóa dữ liệu cho User"
		...
		echo ""
		echo "Chọn loại chứng chỉ bạn muốn sử dụng:"
	...
	echo ""
	echo "---------------------------------------------------------------"
	echo "Quá trình cài đặt OpenVPN sẽ được tiến hành."
	echo "Chúng tôi sẽ tạo một người dùng đầu tiên khi cài đặt hoàn tất !"
	APPROVE_INSTALL=${APPROVE_INSTALL:-n}
	if [[ $APPROVE_INSTALL =~ n ]]; then
		read -n1 -r -p "Bấm phím bất kỳ để tiến hành cài đặt..."
	fi
}
...
ifconfig-pool-persist ipp.txt" >>/etc/openvpn/server.conf
	...
	2) # Self-hosted DNS resolver (Unbound)
		echo 'push "dhcp-option DNS 10.10.10.1"' >>/etc/openvpn/server.conf
		if [[ $IPV6_SUPPORT == 'y' ]]; then
...
verb 3" >>/etc/openvpn/server.conf
	...
	# Script to add rules
	echo "#!/bin/sh
iptables -t nat -I POSTROUTING 1 -s 10.10.10.0/24 -o $NIC -j MASQUERADE
...
ip6tables -I INPUT 1 -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >>/etc/iptables/add-openvpn-rules.sh
	...
	echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s 10.10.10.0/24 -o $NIC -j MASQUERADE
...
function newClient() {
	...
	echo "Tên phải bao gồm ký tự chữ và số. Nó cũng có thể bao gồm một gạch dưới hoặc một dấu gạch ngang."
	echo "---------------------------------------------------------------"
	...
	echo ""
	echo "---------------------------------------------------------------"
	echo "Bạn có muốn bảo vệ cấu hình người dùng bằng mật khẩu không?"
	echo "---------------------------------------------------------------"
	echo "   1) Thêm người dùng mới không cần mật khẩu"
	echo "   2) Thêm người dùng mới có mật khẩu"
	...
	if [[ $CLIENTEXISTS == '1' ]]; then
		...
		1)
			...
			;;
		2)
			echo "⚠️ Bạn cần nhập mật khẩu người dùng mới ⚠️"
			echo "---------------------------------------------------------------"
...
function removeOpenVPN() {
	echo ""
	read -rp "Bạn có muốn gỡ cài đặt OpenVPN? [y/n]: " -e -i n REMOVE
	if [[ $REMOVE == 'y' ]]; then
...
function manageMenu() {
	...
	echo "1010101010101010101010101010101010101010101010"
	echo "-----------------------------------------------------------------"
	echo ""
	echo "OpenVPN server đã được cài đặt"
	echo ""
	echo "Bạn muốn làm gì?"
	echo "   1) Thêm mới người dùng"
	echo "   2) Xóa người dùng"
	echo "   3) Gỡ cài đặt OpenVPN"
	echo "   4) Exit"
